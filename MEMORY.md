...
- **2026-07-12** — Added a second `aside.inquiry` in section 04 / Protocols, immediately after the "Infrastructure hypothesis" aside, presenting the card as a portable-record primitive and linking to Card Commons (`https://csandbatch.github.io/card-commons/`, `rel="external"`, anchor text "Card Commons" only). **This is the site's first-ever external link.** No new CSS: the existing `.inquiry`/`.provisional` classes are reused verbatim with new copy and a new `INSTRUMENT / 01` / `EXTERNAL` header pairing (mirrors the existing `HYPOTHESIS / 01` / `PROVISIONAL` pattern). Copy states plainly that Card Commons is a v0.1 proposal, not an adopted standard — no ownership or adoption claim. `app.js`, color tokens, and section numbering untouched.
